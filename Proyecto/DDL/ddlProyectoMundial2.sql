...
CREATE TABLE venta (
    ...
    CONSTRAINT fk_venta_cliente
        FOREIGN KEY (cliente_id) REFERENCES cliente(id) ON
    DELETE
        SET
        NULL,
        CONSTRAINT fk_venta_usuario
        FOREIGN KEY (usuario_id) REFERENCES usuario(id) ON
        DELETE
            SET
            NULL
);
--se agrega para numero de factura 
ALTER TABLE venta ADD COLUMN numero_factura VARCHAR(20) UNIQUE;
...
CREATE TABLE log_usuario (
    ...
    usuario_id INT,
    fecha      TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    accion     VARCHAR(50) NOT NULL CHECK (accion IN ('LOGIN', 'LOGOUT', 'LOGIN_FALLIDO')),
    ip         VARCHAR(45),
...
drop table if exists anulacion_venta cascade; 
